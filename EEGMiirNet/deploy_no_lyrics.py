...
def plot_metrics(train_losses, val_losses, val_accuracies, num_epochs,fold,run):
    ...
    plt.tight_layout()
    filename = f'plot_fold_{fold}_run_{run}_no_lyrics.png'
...
data = []
labels = []
file_path = 'processed_data/data_no_lyrics.h5'
...
for i, (train_index, test_index) in enumerate(logo.split(data,labels,groups)):
    ...
    train_losses, val_losses, val_accuracies, model_params, guesses, actuals = train_validate(train_loader,test_loader,num_epochs,0.01)
...
filename = f'confusion_matrix_run{run}_no_lyrics.png'
plt.savefig(filename)
plt.show()
for i, model in enumerate(best_models):
    model_file = f'BestModel{run}_Fold{i+1}_no_lyrics.pt'
